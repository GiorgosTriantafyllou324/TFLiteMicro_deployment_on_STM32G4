...
TfLiteStatus MicroInterpreterGraph::InvokeSubgraph(int subgraph_idx) {
  unsigned long int tick[10];
  int tick_counter=0;
  const char *registrations[10];
  int previous_subgraph_idx = current_subgraph_index_;
  current_subgraph_index_ = subgraph_idx;
...
// only defined for builds with the error strings.
    ...
    //MicroPrintf("Node %s : %lu (ms)", OpNameFromRegistration(registration), tick[tick_counter]-tick[tick_counter-1]);
